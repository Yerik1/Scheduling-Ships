...
int scheduler_sjf(ReadyQueue *q){
    ...
    for (int i = 1; i < q->count; i++){
        if (q->tasks[i]->ship.burstTime < q->tasks[best]->ship.burstTime){
            best = i;
        }
    }
    return best;
...
int scheduler_strn(ReadyQueue *q){
    ...
    for (int i = 0; i < q->count; i++){
        ...
        if (q->tasks[i]->ship.remainingTime <= 0) {
            continue;
        }

        if (best == -1 || q->tasks[i]->ship.remainingTime < q->tasks[best]->ship.remainingTime){
            best = i;
        }
    }
...
int scheduler_rr(ReadyQueue *q, int quantum, int *rr_index){
    ...
    int ticks_used = (task->ship.burstTime - task->ship.remainingTime + 1);

    // Rotate to the next ship if the current one is finished or has used up its quantum
    int will_finish = (task->ship.remainingTime <= 1);
